...
PFMIFSCALLBACK GreenteaFSChkdskCallback = NULL;
ULONG FsCheckFlags;
...
NTSTATUS
NTAPI
GreenteaFSFormat(IN PUNICODE_STRING DriveRoot,
...
//             PartitionInfo.PartitionType == PARTITION_FAT32_XINT13)
    {
        /* FAT32 */
        Status = GreenteaFSPerformFormat(FileHandle,
                             &PartitionInfo,
                             &DiskGeometry,
                             Label,
                             QuickFormat,
...
VOID
GreenteaFSUpdateProgress(PFORMAT_CONTEXT Context,
               ULONG Increment)
{
    ULONG NewPercent;
...
VOID
GreenteaFSPrintV(PCHAR Format, va_list args)
{
    TEXTOUTPUT TextOut;
    CHAR TextBuf[512];
    ...
    /* Do the callback */
    if (GreenteaFSChkdskCallback)
        GreenteaFSChkdskCallback(OUTPUT, 0, &TextOut);
}


VOID
GreenteaFSPrint(PCHAR Format, ...)
{
    va_list args;

    va_start(args, Format);
    GreenteaFSPrintV(Format, args);
    va_end(args);
}
